...
def login():
    ...
    if request.method == 'POST':
        ...
        if user is None:
            ...
            login_user(user)
            login_manager.login_message = u"로그인 성공!"
            return redirect(url_for('main.home'))
...
def register():
    if request.method == 'POST':
        ...
        if username is not None and any(char.isdigit() for char in username) or any(char in special_char for char in username):
        ...
        user=User(user_email=user_email,pw=generate_password_hash(password1),username=username)
        db.session.add(user)
        db.session.commit()
        flash('회원가입 성공')
        return redirect('/')

...
from models import BookComment, BookInfo, BookRentInfo, User
...
def home():
    ...
    if request.method == "POST":
        ...
        # bookId랑 내가 불러올 rentInfo의 book_id랑 같으면? 해당 책을 이미 빌렸다고 flash뿌림
            ...
            rent = BookRentInfo(
                book_id=bookId, user_id=current_user.id, rent_date=date.today()
            )
            db.session.add(rent)
            book.stock -= 1
            db.session.commit()
...
def books(book_id):
    ...
    if request.method == "POST":
        ...
        db.session.add(book)
        db.session.commit()
    comments = BookComment.query.filter_by(bookInfo_id=book.id).order_by(
        desc(BookComment.id)
    )
...
def book_return():
    ...
    if request.method == "POST":
        ...
        user_bookRentInfo = BookRentInfo.query.filter_by(
            id=rent_id, user_id=current_user.id
        ).first()
